<template lang="">
    ...
    <div class = "con_container">
        <div class="con_top">
          ...
          <form action="">
            <input name = "keyword" type ="search" class = "repo_search" placeholder="즐겨찾기 검색" v-model="searchStr" v-on:keyup.enter="search"></input>
            <input name = "tab" :value="this.$route.query.tab" hidden/>
            <input type="submit" hidden/>
          </form>
            
        </div>
        <div  class="repo_div" v-for="data in searchResult">
          <div class="repo_div_con" @click="getClick(data.member_nick,data.repo_name)">
            ...
            <div class="unStar_div" v-if="starbtn" >
              <div class="btns" @click="deleteStar(data.repo_idx)">  <font-awesome-icon icon="fa-regular fa-star" /> 삭제</div>
            </div>
          </div>
...
export default {
  data() {
    return {
      ...
      pageCount: 10,
      idx: 0,
      starbtn: this.$route.params.nick == store.getters.getUserNick,
    };
  },
  components: {
    TimeAgo,
