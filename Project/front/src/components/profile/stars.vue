<template lang="">
    <div class="stars_container">
        <div class="stars_div" v-for="(star,idx) in stars" v-bind:key="idx">
            <div>
              <button type="button" class="star_btn"  @click="[insertStar(star.repo_name),refreshAll()]">
                <svg aria-hidden="true" height="16" viewBox="0 -2 16 16" version="1.1" width="16" data-view-component="true" class="star_svg">
                        <path fill-rule="evenodd" d="M8 .25a.75.75 0 01.673.418l1.882 3.815 4.21.612a.75.75 0 01.416 1.279l-3.046 2.97.719 4.192a.75.75 0 01-1.088.791L8 12.347l-3.766 1.98a.75.75 0 01-1.088-.79l.72-4.194L.818 6.374a.75.75 0 01.416-1.28l4.21-.611L7.327.668A.75.75 0 018 .25zm0 2.445L6.615 5.5a.75.75 0 01-.564.41l-3.097.45 2.24 2.184a.75.75 0 01.216.664l-.528 3.084 2.769-1.456a.75.75 0 01.698 0l2.77 1.456-.53-3.084a.75.75 0 01.216-.664l2.24-2.183-3.096-.45a.75.75 0 01-.564-.41L8 2.694v.001z"></path>
                </svg> 
                Star
              </button>
              <button type="button" class="starred_btn"  @click="[deleteStar(star.repo_name),refreshAll()]">
                <svg aria-hidden="true" height="16" viewBox="0 0 16 16" version="1.1" width="16" data-view-component="true" class="starred_svg">
                    <path fill-rule="evenodd" d="M8 .25a.75.75 0 01.673.418l1.882 3.815 4.21.612a.75.75 0 01.416 1.279l-3.046 2.97.719 4.192a.75.75 0 01-1.088.791L8 12.347l-3.766 1.98a.75.75 0 01-1.088-.79l.72-4.194L.818 6.374a.75.75 0 01.416-1.28l4.21-.611L7.327.668A.75.75 0 018 .25z"></path>
                </svg>
                Starred
              </button>
            </div>
            <div class="stars_repository">
              
                <a :href="'/'+star.member_nick+'/'+star.repo_name">{{star.member_nick}} / {{star.repo_name}}</a>
            </div>

           

            <div class="stars_repository_detail">
                협업 사이트
            </div>

            
         
            <svg aria-label="star" role="img" height="16" viewBox="0 0 16 16" version="1.1" width="16" data-view-component="true" class="octicon octicon-star">
                <path fill-rule="evenodd" d="M8 .25a.75.75 0 01.673.418l1.882 3.815 4.21.612a.75.75 0 01.416 1.279l-3.046 2.97.719 4.192a.75.75 0 01-1.088.791L8 12.347l-3.766 1.98a.75.75 0 01-1.088-.79l.72-4.194L.818 6.374a.75.75 0 01.416-1.28l4.21-.611L7.327.668A.75.75 0 018 .25zm0 2.445L6.615 5.5a.75.75 0 01-.564.41l-3.097.45 2.24 2.184a.75.75 0 01.216.664l-.528 3.084 2.769-1.456a.75.75 0 01.698 0l2.77 1.456-.53-3.084a.75.75 0 01.216-.664l2.24-2.183-3.096-.45a.75.75 0 01-.564-.41L8 2.694v.001z"></path>
            </svg> 
            <span class="stars_sub">{{star.star_count}} </span>
            <span class="stars_sub">{{star.push_date==0?"오늘":+star.push_date+'일 전'}} </span>
        
        </div>
        
    </div>

</template>
<script>
import axios from 'axios';
import store from '../../vuex/store';
export default {
  data() {
    return {
      stars: [],
      
     
      
    };
  },
  methods:{
    refreshAll() {
            // 새로고침
            this.$router.go();
        },
    selectStarlist(){
      axios
      .get("/api/selectStarlist",{
        params:{
          nick: this.$route.params.nick,
          idx: this.$route.params.idx,
         
        },
      })
      .then((response) => {
        this.stars = response.data;
        console.log(this.stars)

      })
      
      
      
    },
      insertStar(reponame){
        
            axios
            .get("/api/insertStar", {
                params: {
                    reponame:reponame,
                    idx:store.getters.getUserIdx,
                },
                })
                .then((response) => {
                // handle success
                
                
                })
                .catch((error) => {
                // handle error
                console.log(error);
                })
                .finally(() => {
                // always executed
                });
                
            },
        deleteStar(reponame){
          
            axios
            .get("/api/deleteStar", {
                params: {
                    reponame:reponame,
                    idx:store.getters.getUserIdx,
                },
                })
                .then((response) => {
                // handle success
                
                
                })
                .catch((error) => {
                // handle error
                console.log(error);
                })
                .finally(() => {
                // always executed
                });
                
            },

            
    // selectstarcount(idx){
    //   alert(this.stars[idx].repo_name)
    //   axios
    //   .get("/api/selectstarcount",{
    //     params:{
    //       reponame: this.stars[idx].repo_name,
    //       idx: store.getters.getUserIdx,
    //     },
    //   })
    //   .then((response) => {
    //     this.starcount = response.data;
        

    //   })
    // },

   
  },
  mounted(){
    this.selectStarlist();
    
    
  }
}
</script>

<style lang="sass">
@import 'src/assets/sass/profile/stars'
</style>       